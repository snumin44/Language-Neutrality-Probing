...
def argument_parser():
    ...
    parser = argparse.ArgumentParser(description='sentence identification train')
...
def train(encoder, probe_model, train_dataloader, valid_dataloader, optimizer, scheduler, args):
    ...
    for epoch_i in range(args.epochs):           
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            hidden_states_summed = encoder(input_ids, attention_mask, token_type_ids, args.probe_layer, pooler=args.pooler)
        ...
        for _, batch in enumerate(valid_dataloader):
            ...
            total_valid_accu += get_accuracy(logits, labels)
             
        avg_valid_loss = total_valid_loss / len(valid_dataloader)
        avg_valid_accu = total_valid_accu / len(valid_dataloader)
        ...
        print("[epoch {}] average valid accuracy:{}".format(epoch_i+1, avg_valid_accu))        
    ...
    if args.probe_type == 'cumulative_scoring':
        model_path = os.path.join(output_dir,
                                  "sentence_identification_{}_layer-1-{}_pooler-{}_epoch-{}.pt".format(str(args.probe_type),
                                                                                                       str(args.probe_layer),
                                                                                                       str(args.pooler),
                                                                                                       str(epoch_step)))   
    else:
        model_path = os.path.join(output_dir,
                                  "sentence_identification_{}_layer-{}_pooler-{}_epoch-{}.pt".format(str(args.probe_type),
                                                                                                     str(args.probe_layer),
                                                                                                     str(args.pooler),
                                                                                                     str(epoch_step)))           
...
def main(args):
    ...
    train_dataset = SentenceClassification_Dataset.load_data(args.train_dir)
    valid_dataset = SentenceClassification_Dataset.load_data(args.valid_dir)
    ...
    probe_model = Sentence_Classifier(encoder.hidden_size, len(train_dataset.lang_id)).to(args.device)

    LOGGER.info('*** Sentence Identificatoin ***')
    LOGGER.info('Probe Type: \'{}\''.format(str(args.probe_type)))
    LOGGER.info('Probe Layer: {}'.format(str(args.probe_layer)))
    ...
    # Freeze Parameters of BERT # electra_head.head.weight
